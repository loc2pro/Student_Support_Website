...
import { getListCourses } from "../actions/coursesAction";
...
function CoursesView() {
  ...
  const { isLoading, courses, course, semester } = useSelector(
    (state) => state.listCourses
  );
  const { semesters } = useSelector((state) => state.listSemester);
  const [menuOpen, setMenuOpen] = React.useState(false);
  useEffect(() => {
    const data = dispatch(getListCourses());
    ...
    "
      ...
      {semesters.length > 0 && (
        <CreateCourses
          ...
          setToast={setToast}
          semester={semester ? semester : semesters[0]}
        />
      )}
      {course && (
        <UpdateCourses
