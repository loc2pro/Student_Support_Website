...
import { getListSemester } from "../actions/semesterAction";
...
function SemesterView() {
  ...
  const { isLoading, semesters, semester, planstudy } = useSelector(
    (state) => state.listSemester
  );
  const { planstudys } = useSelector((state) => state.listPlanStudys);
  const [menuOpen, setMenuOpen] = React.useState(false);
  useEffect(() => {
    const data = dispatch(getListSemester());
  ...
  useEffect(() => {
    dispatch(getListPlanStudy());
  }, []);
  const [selected, setSelected] = useState(null);
  const [fileName, setFileName] = useState("");
  useEffect(() => {
    if (!planstudy) {
      setSelected(planstudys[0]);
    } else {
      setSelected(planstudy);
    }
    setFileName(`${selected ? selected.tenkehoach : ""}`);
  }, [planstudys]);
          ...
          <div className="container mx-auto max-w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4">
              <StatusCard
                color="pink"
                icon="trending_up"
                title="Số lương học kỳ"
                amount={semesters.length}
                percentage="3.48"
    ...
    "
      ...
      {planstudys.length > 0 && (
        <CreateSemester
          ...
          setToast={setToast}
          planstudy={planstudy ? planstudy : planstudys[0]}
        />
      )}
      {semester && (
        <UpdateSemester
